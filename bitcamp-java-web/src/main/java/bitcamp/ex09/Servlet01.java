...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    // => 웹 브라우저에서 '세션 아이디'를 제고앟면,
    //    getSession()을 호출할 때 기존에 생성했던 세선 객체를 리턴한다.
    //    즉 이미 이 클라이언트를 위해 만든 객체가 있다면 그 객체를 리턴한다.
    HttpSession session = request.getSession();
    session.setAttribute("v2", "bbb뿌뿌");
    
    // 3) ServletRequest 보관소에 값 넣기
    // => ServletRequest 객체는 클라이언트가 요청할 때마다 생성된다.
    request.setAttribute("v3", "ccc하이");
    
    response.setContentType("text/plain;charset=UTF-8");
    PrintWriter out = response.getWriter();
    
    out.println("보관소에 값을 넣었습니다.메롱 - /ex09/s1");
