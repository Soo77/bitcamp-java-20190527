...
public class BoardCommand {
  ...
  public void add(ServletRequest request, ServletResponse response) {
    PrintWriter out = response.getWriter();
    out.println("<html><head><title>게시물등록</title>"
  ...
  public void delete(ServletRequest request, ServletResponse response) {
    PrintWriter out = response.getWriter();
    out.println("<html><head><title>게시물삭제</title>"
        + "<meta http-equiv='Refresh' content='1;url=/board/list'>" + "</head>");
  ...
  public void list(ServletRequest request, ServletResponse response) {
    ...
    try {
      ...
      for (Board board : boards) {
        out.printf(
            "<tr><th>%s</th>" + "<th><a href='/board/detail?no=%d'>%s</a></th>" + "<th>%s</th>"
                + "<th>%d</th>" + "</tr>\n",
