...
public class LessonCommand {
  ...
  public void detail(ServletRequest request, ServletResponse response) {
    ...
    try {
      ...
      if (lesson == null) {
        ...
        out.printf("총 수업시간: <textarea name='totalHours' rows='5' cols='50'>%s</textarea><br>\n", lesson.getTotalHours());
        out.printf("일 수업시간: <textarea name='dayHours' rows='5' cols='50'>%s</textarea><br>\n", lesson.getDayHours());
  ...
  public void list(ServletRequest request, ServletResponse response) {
    ...
    try {
      ...
      out.println("<tr><th>번호</th><th>수업명</th><th>설명</th><th>시작일</th>"
          + "<th>종료일</th><th>총 수업시간</th><th>일 수업시간</th></tr>");

      List<Lesson> lessons = lessonDao.findAll();
      for (Lesson lesson : lessons) {
        out.printf("<tr><th>%d</th>" 
                  + "<th><a href='/lesson/detail?no=%d'>%s</a></th>" + "<th>%s</th>"
                  + "<th>%s</th>" + "<th>%s</th>" + "<th>%d</th>"+ "<th>%d</th>" + "</tr>\n",
